window.addEventListener("load", () => {
  ...
  <line x1="32" y1="28" x2="32" y2="3" stroke-linecap="butt" stroke-width="6" stroke="${fg}"/></g></defs>
    ...
    let initSwitches = (el) => {
        ...
        el.refresh = () => {
            ...
            if (src) el.style.backgroundImage = "url(" + src + ")";
                ...
                let svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${w}" height="${h * 2}" viewBox="0 0 ${w} ${
                    h * 2
    ...
    let initKnobs = (el) => {
        ...
        ik.pointerdown = (ev) => {
            ...
            if (ik.itype == "k" && op.knobMode == "circularabs") {
                let dv = ik.valrange.min + ((da / Math.PI) * 0.75 + 0.5) * (ik.valrange.max - ik.valrange.min);
                el.setValue(dv);
            }
            if (ik.itype != "k" && op.sliderMode == "abs") {
                let dv =
                    (ik.valrange.min + ik.valrange.max) * 0.5 +
                    ((dx - cx) / ik.sensex - (dy - cy) / ik.sensey) * (ik.valrange.max - ik.valrange.min);
                el.setValue(dv);
            }
            ik.dragfrom = {
                x: ev.clientX,
